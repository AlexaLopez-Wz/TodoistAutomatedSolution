...
test("As an user, I should be able to log in successfully using email by providing valid credentials", async t => {
...
test("As an user, I should not be able to login by providing a wrong email format", async t => {
    ...
    await t.expect(mainPage.todayView.exists).notOk()
})
